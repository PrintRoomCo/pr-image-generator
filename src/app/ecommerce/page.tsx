...
import { getEcommerceTotalImages } from '@/types/ecommerce'
...
export default function EcommercePage() {
  ...
  useEffect(() => {
    fetch('/api/jobs?jobType=ecommerce&limit=10')
      .then(res => res.json())
      .then(data => {
  ...
  function addFiles(fileList: FileList | null) {
    ...
    setUploads(current => {
      ...
      if (mode === 'multi-angle-product' && !next.some(upload => upload.isPrimary) && next[0]) {
  ...
  function removeUpload(uploadId: string) {
    setUploads(current => {
      ...
      if (mode === 'multi-angle-product' && next.length > 0 && !next.some(upload => upload.isPrimary)) {
  ...
  function handleModeChange(nextMode: EcommerceGenerationMode) {
    ...
    if (nextMode === 'multi-angle-product') {
      setUploads(current => {
        ...
        if (current.some(upload => upload.isPrimary)) return current
  ...
  async function uploadSourceImages(): Promise<PersistedUpload[]> {
    ...
    const uploadRes = await fetch('/api/ecommerce/uploads', {
      method: 'POST',
      body: formData,
    })
  ...
  function buildConfig(persistedUploads: PersistedUpload[]): EcommerceGenerationConfig {
    const uploadsByClientId = new Map(
      persistedUploads.map(upload => [upload.clientId, upload])
    )
  ...
  async function handleGenerate() {
    ...
    if (!prompt.trim()) {
      setErrorMessage('Add a prompt for Replicate to follow.')
      return
    }

    if (mode === 'multi-angle-product' && !uploads.some(upload => upload.isPrimary)) {
      setErrorMessage('Choose a primary image for the multi-angle product set.')
      return
    }
    ...
    try {
      ...
      if (!createRes.ok) {
        throw new Error(createData.error || 'Failed to create ecommerce job')
      }

      setActiveJob(createData.job)
      await fetch('/api/process?jobType=ecommerce', { method: 'POST' })
    } catch (error) {
  ...
  return (
    <div>
      <Header
        title="Ecommerce Images"
        description="Upload product photos, add a Replicate prompt, and generate print-room ready ecommerce imagery."
      />
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
        <div className="space-y-6 lg:col-span-2">
          <Card className="p-6">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  Upload up to {MAX_ECOMMERCE_UPLOADS} JPG, PNG, or WebP images.
                </p>
              </div>
              <div className="flex flex-wrap gap-2">
                <Button
                  type="button"
                  variant={mode === 'separate-products' ? 'default' : 'outline'}
                  onClick={() => handleModeChange('separate-products')}
                >
                  Separate products
                </Button>
                <Button
                  type="button"
                  variant={mode === 'multi-angle-product' ? 'default' : 'outline'}
                  onClick={() => handleModeChange('multi-angle-product')}
                >
                  Multi-angle product
                </Button>
              </div>
            ...
            <button
              type="button"
              className={`mt-6 flex min-h-48 w-full flex-col items-center justify-center rounded-xl border border-dashed px-6 text-center transition-colors ${
            ...
            >
              ...
              <p className="mt-4 text-xs text-muted-foreground">
                Best results come from clear, well-lit product photos.
              </p>
            </button>
                  ...
                  <Card key={upload.id} className="overflow-hidden">
                    ...
                    <div className="space-y-3 p-3">
                      <div className="flex items-start justify-between gap-3">
                        ...
                        {upload.isPrimary && mode === 'multi-angle-product' && (
                          <Badge>Primary</Badge>
                        )}
                      </div>
                      <div className="flex gap-2">
                        {mode === 'multi-angle-product' && (
                          <Button
                            type="button"
                            size="sm"
                            variant={primaryUploadId === upload.id ? 'default' : 'outline'}
          ...
          <Card className="p-6">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Tell Replicate how to style or adapt the uploaded product for The Print Room.
            </p>
            <Textarea
              value={prompt}
              onChange={event => setPrompt(event.target.value)}
              placeholder="Example: Create a premium hero image with soft studio lighting, preserve the garment exactly, and make it suitable for a print-room campaign landing page."
              className="mt-4"
            />
          </Card>
        </div>
        ...
        <div className="space-y-6">
          ...
          <Card className="p-4 text-sm">
            <div className="flex items-center justify-between">
              ...
              <Badge variant="info">
                {mode === 'multi-angle-product' ? 'Multi-angle product' : 'Separate products'}
              </Badge>
            </div>
            ...
            <div className="mt-2 flex items-center justify-between">
              ...
              <span className="font-medium">{selectedTypes.size}</span>
            </div>
          </Card>
          ...
          <Button
            ...
            disabled={uploads.length === 0 || selectedTypes.size === 0 || !prompt.trim() || generating}
            className="w-full"
            size="lg"
          >
      ...
      <div className="mt-12">
        ...
        {loading ? (
          ...
          <JobList jobs={recentJobs} basePath="/ecommerce" />
        )}
      </div>
    </div>
  )
